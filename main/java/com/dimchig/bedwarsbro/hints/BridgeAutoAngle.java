...
public class BridgeAutoAngle {
    ...
    public static void aim() {
    	...
    	rotateTo(player, direction, rotation_godbridge_pitch);
    }
    ...
    public static void rotateTo(Entity player, float target_angle_yaw, float target_angle_pitch) {
        ...
        if (target_angle_yaw != player.rotationYaw || target_angle_pitch != player.rotationPitch) {
        	...
        	if (prop_show_chat_messages == true) {
        		int yaw = (int)target_angle_yaw % 180;
        		ChatSender.addText(mod_prefix + "&fУгол установлен в (&b" + yaw + ", " + target_angle_pitch + "&f)");
        	}
        }
    }
