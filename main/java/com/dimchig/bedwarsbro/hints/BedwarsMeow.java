...
public class BedwarsMeow {
	...
	public class MeowMsg {
		...
		public String getText(String variable) {
			...
			if (this.msgcase == MsgCase.KILL || this.msgcase == MsgCase.FINAL_KILL || this.msgcase == MsgCase.DEATH) {
				String killer = "игрок";
			...
			} else if (this.msgcase == MsgCase.WIN || this.msgcase == MsgCase.GAME_START) {
				new_text = this.text;
			} else {
				return null;
			}
	...
	public void readFile() {		
		...
		try {
			...
			String[] splitters_text = new String[] {
					category_kill_messages, category_final_kill_messages, category_death_messages, 
					category_bed_messages, category_wins_messages, category_game_start_messages, "randomtexttoletmycodeparselastline"
			};
			MsgCase[] splitters_cases = new MsgCase[] {
					MsgCase.KILL, MsgCase.FINAL_KILL, MsgCase.DEATH, 
					MsgCase.BED_SINGLE, MsgCase.WIN, MsgCase.GAME_START
			};
	...
	public String getMeows(MsgCase msgcase) {
		...
		if (msgcase == MsgCase.KILL || msgcase == MsgCase.FINAL_KILL) {
		...
		} else if (msgcase == MsgCase.WIN || msgcase == MsgCase.GAME_START) {
			variable = "";
		}
	...
	public void initMeowMessages(boolean isRemoveBadWords) {
		...
		String[] killMessages = new String[] {
			...
			String[] finalKillMessages = new String[] {
					...
					"Я просто бог в этой игре!",
					"Поймал на ашибке!",
					"А что, мой маленький, без кроватки сложно получилось, игрок?"
			};
		...
		String[] splitters_text = new String[] {
				category_kill_messages, category_final_kill_messages, category_death_messages, 
				category_bed_messages, category_wins_messages, category_game_start_messages
		};
		String[][] splitters_messages = new String[][] {
				killMessages, finalKillMessages, deathMessages, 
				bedMessages, winMessages, gameStartMessages
		};
